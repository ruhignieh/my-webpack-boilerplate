const fs = require('fs');
const { resolve, join, parse, isAbsolute, sep } = require('path');
const mkdirp = require('mkdirp');
const inquirer = require('inquirer');
const { template } = require('lodash');
const { dir: templatePath, init } = require('./config');
...
const create = options => {
    ...
    filesTypes.forEach(fileType => {
        ...
        fileType === 'xml' && (fileType = options.xml || 'axml');
        ...
        const notStyleFiles = ['js', 'axml', 'json', 'pug'];
        const templateExt = notStyleFiles.includes(fileType) ? fileType : 'acss';
        const templatePathURL = type === 'page' ? pageTemplatePath : componentTemplatePath;
        const templateFile = fs.readFileSync(resolve(templatePathURL, `template.${templateExt}`), 'utf8');
...
const createPromptItems = options => ([
    {
        ...
        validate: answer => {
            ...
            try { return validateAppJson(rootDir); } catch (err) { return err.message; }
        },
    },
    {
        message: '请选择需要创建的类型',
...
const createBuilder = yargs => {
    yargs
        .usage('\n create-page [args] | create-page create [args]')
        .options({
            t: {
                alias: 'type',
